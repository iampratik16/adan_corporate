...
async function main(): Promise<void> {
  ...
  /*
   * The manifest the site reads. Written here because this is the only place
   * that knows what was built.
   *
   * src/app/page.tsx used to answer "is there a film?" by shelling out to
   * ffprobe at build time, once per rung. That works locally and fails silently
   * everywhere else: Vercel's build image has no ffmpeg, execFileSync throws,
   * and the catch dropped every rung. The deploy served the poster and no video
   * at all, while the mp4 files sat on the CDN answering 200 to anyone who
   * asked for them directly. Nothing in the build logs said a word.
   *
   * A generated content file is a static read with no external binary and no
   * catch to swallow, and it behaves identically on every machine.
   */
  const rungs = FILM_RUNGS.map((r) => `hero-film-${r.width}.mp4`).filter((name) =>
    existsSync(path.join(OUT, name)),
  );
  await writeFile(
    path.join(ROOT, 'content/hero-film.ts'),
    [
      ...
      `  rungs: [${rungs.map((r) => `'${r}'`).join(', ')}] as const,`,
      '} as const;',
      '',
    ].join('\n'),
    'utf8',
  );
  console.log(`  Manifest content/hero-film.ts  ${rungs.length} rung(s), ${seconds.toFixed(2)}s`);
